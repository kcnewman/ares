...
@lru_cache(maxsize=1)
def _get_feature_pipeline() -> dict:
    config = load_config()
    return make_feature_pipeline(config)


def _load_volatility_thresholds(
    pipeline: dict, spread: pd.Series
) -> tuple[float, float]:
    q25 = float(pipeline["global_ref"].get("volatility_q25", np.nan))
    q75 = float(pipeline["global_ref"].get("volatility_q75", np.nan))
    if np.isfinite(q25) and np.isfinite(q75) and q75 > q25:
        return q25, q75

    loc_iqr_values = pd.Series(list(pipeline["loc_iqr"].values()), dtype=float)
...
def predict(
    ...
    try:
        ...
        transformed_data = transform_features(input_data, feature_pipeline)
